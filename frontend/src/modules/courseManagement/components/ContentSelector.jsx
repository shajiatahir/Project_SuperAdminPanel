...
const ContentSelector = ({ courseId, onClose, onContentAdded }) => {
    ...
    const handleAdd = async (content, type) => {
        try {
            ...
            const updatedCourse = await handleAddContent(courseId, newContent);
            if (updatedCourse && onContentAdded) {
                onContentAdded(updatedCourse.sequence);
                onClose();
            }
        } catch (err) {
